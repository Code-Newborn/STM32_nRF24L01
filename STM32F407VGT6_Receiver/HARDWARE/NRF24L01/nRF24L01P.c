...
HAL_StatusTypeDef HAL_nRF24L01P_SendCommand( nRF24L01P* nRF, uint8_t Command, uint8_t* TxBuffer, uint8_t* RxBuffer, uint8_t Size ) {
    /* ---- Local Vars. ---- */
    uint8_t* tempTxBuffer = ( uint8_t* )malloc( ( Size + 1 ) * sizeof( uint8_t ) );  // 使用 malloc 动态分配内存;
    uint8_t* tempRxBuffer = ( uint8_t* )malloc( ( Size + 1 ) * sizeof( uint8_t ) );
    /* ---- Pre Process ---- */
